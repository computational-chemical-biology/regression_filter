...
def getPoints(dinfo, lst, ppm=20, rttol=10):
    rt = []
    for i in lst.index:
        sel = (np.abs(np.abs(dinfo['mz']-lst.loc[i, 'Mass Begin'])/dinfo['mz'])*(10**6))<ppm
        if not sel.any():
            continue
        retention_times = dinfo.loc[sel, 'rt']
        ms1 = dinfo.loc[sel, 'ms1']
        xtime = lst.loc[i, 'Time']
        ydiff = np.abs(retention_times-xtime)
        if not (ydiff<rttol).any():
            continue
        yloc = np.where((ydiff.min()==ydiff))[0][0]
        #rt.append(lst.loc[i, 'Time'])
        rt.append(ms1.iloc[yloc])
    return rt
...
def get_mgf_string(spectrum):
    ...
    output_string += "INCHI=" + spectrum['inchi'] + "\n"
